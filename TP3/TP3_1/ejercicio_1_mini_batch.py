...
class capa_red():
    def __init__(self, n_neuronas, n_conexiones, f_activacion):
        ...
        self.bias = np.random.rand(1, n_neuronas) * 2 - 1   # numero aleatorio entre -1 y 1
...
def crear_rn(topologia, f_activacion = (identidad, leaky_relu)):
    ...
    for l, neuronas in enumerate(topologia[: - 1]):     # (desde 0 hasta 2)
        
        # logica capa de salida
        if l == len(topologia) - 2:    
            red_neuronal.append(capa_red(topologia[l+1],topologia[l],f_activacion[0]))  # formato capa_red (neuronas, conexiones, f_act)
        
        # logica capas ocultas
        else:
            red_neuronal.append(capa_red(topologia[l+1],topologia[l],f_activacion[1]))
        
    return red_neuronal

def train(red_neuronal, X_train, Y_train, X_test, Y_test, f_costo, batch_size = 32, lr = 0.05, epochs = 1):
    ...
    for epoch in range(epochs):
        ...
        # mezclado de datos
        permutacion = np.random.permutation(m_total)
        X_shuffled = X_train[permutacion]
        Y_shuffled = Y_train[permutacion]

        for i in range(0, m_total, batch_size):
            
            # Cortamos el batch actual
            X_batch = X_shuffled[i : i + batch_size]
            Y_batch = Y_shuffled[i : i + batch_size]

            # forward pass

            current_signal = X_batch

            historial = [(None, X_batch)] # tiene la forma (z,a). z es none al principio porque no tienen pesos las entradas
            ...
            for l in reversed(range(0, len(red_neuronal))):
                ...
                if l == len(red_neuronal) - 1:
                    ...
                    delta.insert(0, f_costo[1](a, Y_batch) * red_neuronal[l].f_activacion[1](a))
            ...
            for l in range(len(red_neuronal)):
                ...
                red_neuronal[l].w = red_neuronal[l].w - ((entrada.T @ delta[l])/ m) * lr

                red_neuronal[l].bias = red_neuronal[l].bias - np.mean(delta[l], axis=0, keepdims=True) * lr

        # error de la epoch
        Y_pred_train = predict(red_neuronal, X_train)
        error_train = f_costo[0](Y_pred_train, Y_train)
        
        Y_pred_test = predict(red_neuronal, X_test)
        error_test = f_costo[0](Y_pred_test, Y_test)
...
errores_train, errores_test = train(mi_red, X_train, Y_train, X_test, Y_test, f_costo, batch_size=32, lr=0.005, epochs=1000)
