...
velocidad.f_triangular('MN', np.deg2rad(-450), np.deg2rad(-300), np.deg2rad(-150))
...
velocidad.f_triangular('MP', np.deg2rad(150),  np.deg2rad(300),  np.deg2rad(450))
...
def simular_animado(t_max, delta_t, theta_0, v_0, a_0):
    ...
    for t in x_tiempo:
        if abs(theta) > np.deg2rad(179) and abs(v_ang) < 0.5:   #Ojo con esto que le pusimos. no es controlador difuso. Solo esta para sacarlo de la quietud de la indeterminación de los 180°
            fuerza_aplicada = 100.0 
            
        else:
            fuerza_aplicada = controlador.inferir(theta, v_ang)
...
simular_animado(10, 0.001, 80, 0, 0)